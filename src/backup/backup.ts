...
export type WorkbenchBackupOptions = {
...
export function setupWorkbenchBackup(
    options: WorkbenchBackupOptions
): void {
...
async function restoreWithRollback(
    ...
    try {
        ...
        await replaceWorkbenchSnapshot(
            importedSnapshot
        );
        ...
        console.error(
            "バックアップ復元中にエラーが発生しました。元データへ戻します。",
            restoreError
        );
        ...
        try {
            ...
            console.error(
                "元データへのロールバックにも失敗しました。",
                rollbackError
            );


            throw new Error(
                "復元に失敗し、元の保存データへの復旧にも失敗しました。"
            );
        }
...
function createExportSuccessMessage(
    snapshot: WorkbenchBackupSnapshot
): string {
