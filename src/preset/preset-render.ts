...
export type PresetGridHandlers = {
 ...
 * IndexedDB読込失敗時。
 */
export function renderPresetGridError(
    container: HTMLElement,
    message: string
): void {
...
function openPresetPdf(
    presetDocument: PresetDocument
): void {

    const objectUrl =
        URL.createObjectURL(
            presetDocument.pdfBlob
        );
