...
import {
    createPresetDocumentId,
    getPresetOrientationLabel,
    isPdfFile,
    isPreviewImageFile,
    PresetDocument,
    PresetOrientation
...
export type PresetManagerOptions = {
...
export type PresetManagerController = {
...
export function setupPresetManager(
    ...
    const listView =
        requireElement<HTMLElement>(
            "#preset-manager-list-view"
        );


    const addButton =
        requireElement<HTMLButtonElement>(
            "#preset-manager-add-button"
        );
    ...
    const editorBackButton =
        requireElement<HTMLButtonElement>(
            "#preset-editor-back-button"
        );


    const editorHeadingTitle =
        requireElement<HTMLElement>(
            "#preset-editor-title"
        );
    ...
    const previewInput =
        requireElement<HTMLInputElement>(
            "#preset-preview-input"
        );


    const currentPreview =
        requireElement<HTMLElement>(
            "#preset-current-preview"
        );
    ...
    const cancelButton =
        requireElement<HTMLButtonElement>(
            "#preset-editor-cancel-button"
        );


    const saveButton =
        requireElement<HTMLButtonElement>(
            "#preset-editor-save-button"
        );
    ...
    function openEditEditor(
        ...
        currentPdf.textContent =
            [
                ...
                `(${formatBytes(
                    presetDocument.pdfBlob.size
                )})`
            ].join(" ");


        if (
            presetDocument.previewBlob &&
            ...
            currentPreview.textContent =
                [
                    ...
                    presetDocument.previewFileName,
                    `(${formatBytes(
                        presetDocument.previewBlob.size
                    )})`
                ].join(" ");
    ...
    async function handleSubmit(
        ...
        if (
            ...
            showEditorError(
                "プレビュー画像はPNG、JPEG、WebPのいずれかを選択してください。"
            );
