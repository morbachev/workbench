 ...
 * バーコード描画時に発生したエラー。
 */
export type InventoryRenderError = {
    rowNumber: number;
    pluCode: string;
    message: string;
};
 ...
 * プレビュー描画結果。
 */
export type InventoryRenderResult = {
    pageCount: number;
    errors: InventoryRenderError[];
};
 ...
 * 1商品行内部のDOM要素。
 */
type InventoryRowElements = {
    nameText: HTMLElement;
    quantityValue: HTMLElement;
    barcode: SVGSVGElement;
    pluText: HTMLElement;
 ...
 */
export function renderInventoryPreview(
    pages: readonly (
...
function createInventoryRow(
    ...
    if (
        ...
        throw new Error(
            "棚卸JAN行テンプレートが正しくありません。"
        );
    }


    const row =
        firstElement.cloneNode(
            true
        );
...
function renderInventoryItem(
    ...
    if (
        ...
        elements.barcode.removeAttribute(
            "title"
        );
    ...
    errors.push({
        ...
        message:
            `${item.rowNumber}行目: PLUコード「${item.pluCode}」 ${barcodeResult.message}`
    });
}
 ...
 * 商品行内部の必須要素を取得する。
 */
function getInventoryRowElements(
    row: HTMLElement
): InventoryRowElements {

    return {

        nameText:
            requireChildElement<HTMLElement>(
                row,
        ...
        quantityValue:
            requireChildElement<HTMLElement>(
                row,
                ".inventory-row__quantity-value"
            ),

        barcode:
            requireChildElement<SVGSVGElement>(
                row,
                ".inventory-row__barcode"
            ),

        pluText:
            requireChildElement<HTMLElement>(
                row,
                ".inventory-row__plu-text"
            ),

        barcodeError:
            requireChildElement<HTMLElement>(
                row,
                ".inventory-row__barcode-error"
            )
    };
}
