...
const barcodeUrl = new URL("gosplan13/", siteRootUrl).href;
const inventoryUrl = new URL("inventory_batch/", siteRootUrl).href;
const discountExclusionUrl = new URL("red_index/", siteRootUrl).href;
const documentBuilderUrl = new URL("document_builder/", siteRootUrl).href;
...
class SiteHeader extends HTMLElement {
     ...
     */
    async copyCurrentPageUrl(button) {
        ...
        try {
            ...
            this.showCopyResult(button, COPY_TOOLTIP_SUCCESS);
        }
        catch (error) {
            console.error("リンクのコピーに失敗しました。", error);
            this.showCopyResult(button, COPY_TOOLTIP_ERROR);
        }
    }
    /**
     ...
     * 一定時間後に元へ戻す。
     */
    showCopyResult(button, message) {
        button.dataset.tooltip = message;
        button.setAttribute("aria-label", message);
        window.setTimeout(() => {
            button.dataset.tooltip =
                COPY_TOOLTIP_DEFAULT;
            button.setAttribute("aria-label", COPY_TOOLTIP_DEFAULT);
