 ...
 *    それ以降は確認ダイアログを表示しない。
 */
import { createExportData, exportToJson, } from "./export.js";
import { restoreImportData, } from "./import.js";
/* =========================================================
   定数
   ========================================================= */
/**
 ...
 */
const STORAGE_KEY = "gosplan13.autosave.v1";
/**
 * 入力停止から自動保存するまでの時間。
 */
 ...
 * 「OK / キャンセル」等になる。
 */
const SAVE_FAILURE_MESSAGE = "自動保存に失敗しました。\n\n"
...
/* =========================================================
   ラベルデータ判定
   ========================================================= */
function isStoredLabelData(value) {
    if (!isObject(value)) {
        return false;
    }
...
function isStoredData(value) {
    ...
    if (!value.labels.every(isStoredLabelData)) {
        return false;
    }
    return true;
}
...
function readStoredData() {
    ...
    try {
        ...
        console.warn("自動保存データを取得できませんでした。", error);
        return null;
    }
    if (!rawData) {
        return null;
    ...
    try {
        ...
        console.warn("自動保存データのJSONが壊れています。", error);
        return null;
    }
    if (!isStoredData(parsedData)) {
        console.warn("自動保存データの形式が現在のGOSPLAN13と一致しません。");
        return null;
    }
    return parsedData;
}
/* =========================================================
   初期化
   ========================================================= */
export function initializeStorage(elements) {
    const { preview, imageModeToggle, } = elements;
    /* =====================================================
       状態
       ===================================================== */
    let saveTimer = null;
     ...
     */
    let isRetryingAfterFailure = false;
    /*
     * このページを開いている間に、
     * 一度でも保存失敗の案内を行ったか。
     *
     ...
     */
    let hasHandledSaveFailure = false;
    /* =====================================================
       保存タイマー解除
       ===================================================== */
    ...
    function retrySaveAfterFailure() {
        ...
        try {
             ...
             * 現在画面上の入力内容には触れない。
             */
            console.warn("自動保存の再試行にも失敗しました。", error);
        }
        finally {
            isRetryingAfterFailure =
                false;
    ...
    function handleSaveFailure(error) {
        console.warn("GOSPLAN13の自動保存に失敗しました。", error);
        /*
         * 再試行中の失敗では、
         * 追加の処理を行わない。
         */
        ...
        hasHandledSaveFailure =
            true;
        const shouldExport = window.confirm(SAVE_FAILURE_MESSAGE);
        /*
         * OKを選択した場合だけ、
        ...
        if (shouldExport) {
            try {
                ...
                console.error("JSON形式での保存にも失敗しました。", exportError);
                window.alert("JSON形式で作業内容を保存できませんでした。\n"
                    + "現在画面に表示されている内容は、"
                    + "そのまま残っています。");
            }
        }
    ...
    function restoreStoredData() {
        ...
        try {
            ...
            console.warn("前回の作業内容を復元できませんでした。", error);
        }
        finally {
            isRestoring =
                false;
    ...
    function handlePreviewInput(event) {
         ...
         * 自動保存対象とする。
         */
        if (!target.closest(".product-label")) {
            return;
        }
        scheduleSave();
    }
    ...
    const imageObserver = new MutationObserver(mutations => {
        ...
        const hasImageChange = mutations.some(mutation => {
            ...
            return target.matches(".product-label__image-preview");
        });
        if (hasImageChange) {
            scheduleSave();
        }
    ...
    function clearStorage() {
        ...
        try {
            ...
            console.warn("自動保存データを削除できませんでした。", error);
        }
    }
    /* =====================================================
       保存データ存在確認
    ...
    preview.addEventListener("change", handlePreviewInput);
    /*
     * 画像あり / 画像なしモード。
     */
    imageModeToggle.addEventListener("change", handleImageModeChange);
    /*
     * Base64画像のsrc変更を監視する。
     */
    ...
    return {
        ...
        destroy: () => {
            ...
            preview.removeEventListener("change", handlePreviewInput);
            imageModeToggle.removeEventListener("change", handleImageModeChange);
