 ...
 * @returns JANコードの処理結果
 */
export function processJanInput(productLabel, janInput) {
    const barcodeSvg = productLabel.querySelector(".product-label__barcode");
    if (!barcodeSvg) {
        throw new Error("商品ラベル内にJANバーコード表示用SVGが見つかりません。");
    }
    const barcodePlaceholder = productLabel.querySelector(".product-label__barcode-placeholder");
    if (!barcodePlaceholder) {
        throw new Error("商品ラベル内にJANバーコード案内表示が見つかりません。");
    }
    /*
     * 前回の入力エラー状態を解除する。
     */
 ...
 * @returns JANコードの処理結果
 */
export function normalizeJan(input) {
    /*
     * 半角空白、全角空白、タブ、改行など、
     * JavaScriptが空白として扱う文字をすべて除去する。
     */
    const valueWithoutWhitespace = removeAllWhitespace(input);
    /*
     * 空白を取り除いた結果が空文字なら、
     * JANコード未設定として正常終了する。
     */
     ...
     * 全角数字を半角数字へ変換する。
     */
    const normalizedValue = convertFullWidthDigitsToHalfWidth(valueWithoutWhitespace);
    /*
    ...
    if (!isHalfWidthDigitsOnly(normalizedValue)) {
        return {
            success: false,
            errorMessage: "JANコードには数字だけを入力してください。"
        };
    }
    /*
     * JANコードは、
    ...
    if (normalizedValue.length !== JAN_BODY_LENGTH &&
        ...
        return {
            success: false,
            errorMessage: "JANコードは12桁または13桁で入力してください。"
        };
    }
    /*
     * 12桁の場合は、
     ...
     */
    if (normalizedValue.length === JAN_BODY_LENGTH) {
        const checkDigit = calculateJanCheckDigit(normalizedValue);
        const completedJan = normalizedValue + String(checkDigit);
        return {
            success: true,
            jan: completedJan,
            displayJan: formatJanForDisplay(completedJan),
            hadCheckDigit: false,
            checkDigit
        };
    }
     ...
     */
    const janBody = normalizedValue.slice(0, JAN_BODY_LENGTH);
    const inputCheckDigit = Number(normalizedValue.slice(-1));
    const calculatedCheckDigit = calculateJanCheckDigit(janBody);
    /*
     * 入力されたチェックデジットと
    ...
    if (inputCheckDigit !== calculatedCheckDigit) {
        return {
            success: false,
            errorMessage: "JANコードのチェックデジットが正しくありません。"
        };
    }
    return {
        success: true,
        jan: normalizedValue,
        displayJan: formatJanForDisplay(normalizedValue),
        hadCheckDigit: true,
 ...
 */
function convertFullWidthDigitsToHalfWidth(value) {
 ...
 */
function isHalfWidthDigitsOnly(value) {
 ...
 */
export function calculateJanCheckDigit(janBody) {
    if (janBody.length !== JAN_BODY_LENGTH) {
        throw new Error("チェックデジットの計算には12桁の数字が必要です。");
    }
    if (!isHalfWidthDigitsOnly(janBody)) {
        throw new Error("チェックデジットの計算対象には半角数字だけを指定してください。");
    }
    let sum = 0;
    for (let index = 0; index < janBody.length; index++) {
 ...
 */
export function formatJanForDisplay(jan) {
    if (jan === "") {
        return "";
    }
    if (jan.length !== JAN_FULL_LENGTH ||
        !isHalfWidthDigitsOnly(jan)) {
        throw new Error("画面表示用の整形には13桁のJANコードが必要です。");
    }
    return [
 ...
 */
export function encodeJan13(jan) {
    if (jan.length !== JAN_FULL_LENGTH ||
        !isHalfWidthDigitsOnly(jan)) {
        throw new Error("バーコード生成には13桁のJANコードが必要です。");
    }
    const firstDigit = jan[0];
    const parityPattern = LEFT_PARITY_PATTERNS[firstDigit];
    if (!parityPattern) {
        throw new Error("JANコード先頭桁のパリティ情報が見つかりません。");
    }
    /*
     * 左端のガードパターン。
     */
    ...
    for (let index = 0; index < 6; index++) {
        ...
        const pattern = parity === "L"
            ? LEFT_ODD_PATTERNS[digit]
            : LEFT_EVEN_PATTERNS[digit];
        if (!pattern) {
            throw new Error("JANコード左側の符号化に失敗しました。");
        }
        encoded += pattern;
    }
    /*
    ...
    for (let index = 7; index < JAN_FULL_LENGTH; index++) {
        ...
        if (!pattern) {
            throw new Error("JANコード右側の符号化に失敗しました。");
        }
        encoded += pattern;
    }
    /*
    ...
    if (encoded.length !== 95) {
        throw new Error("JAN-13バーコードのビット数が正しくありません。");
    }
    return encoded;
}
/**
 ...
 */
export function drawJan13Barcode(barcodeSvg, jan) {
    /*
     * 新しいバーコードを描画する前に、
     * 前回の描画内容をすべて削除する。
     */
    ...
    barcodeSvg.setAttribute("viewBox", `0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`);
    barcodeSvg.setAttribute("preserveAspectRatio", "xMidYMid meet");
    barcodeSvg.setAttribute("shape-rendering", "crispEdges");
    /*
     * 目には表示しないが、
     * スクリーンリーダー向けの説明には
     * JANコードを設定する。
     */
    barcodeSvg.setAttribute("aria-label", `JANバーコード ${formatJanForDisplay(jan)}`);
    /*
     * バーコード背景を白くする。
     *
     * 印刷時や背景色が設定された場合でも、
     ...
     * 黒い縦棒を描画する。
     */
    for (let moduleIndex = 0; moduleIndex < encoded.length; moduleIndex++) {
        if (encoded[moduleIndex] !== "1") {
            continue;
        }
        const rect = document.createElementNS(SVG_NAMESPACE, "rect");
        const x = BARCODE_START_X +
            moduleIndex * MODULE_WIDTH;
        const isGuardModule = isGuardBarModule(moduleIndex);
        const barHeight = isGuardModule
            ? GUARD_BAR_HEIGHT
            : NORMAL_BAR_HEIGHT;
        rect.setAttribute("x", String(x));
        rect.setAttribute("y", String(BAR_START_Y));
        rect.setAttribute("width", String(MODULE_WIDTH));
 ...
 */
function isGuardBarModule(moduleIndex) {
    const isLeftGuard = moduleIndex >= 0 && moduleIndex <= 2;
    const isCenterGuard = moduleIndex >= 45 && moduleIndex <= 49;
    const isRightGuard = moduleIndex >= 92 && moduleIndex <= 94;
    return (isLeftGuard ||
        isCenterGuard ||
        isRightGuard);
}
 ...
 */
export function clearJanBarcode(barcodeSvg) {
    barcodeSvg.replaceChildren();
    barcodeSvg.setAttribute("aria-label", "JANバーコード未設定");
}
