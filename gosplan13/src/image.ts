...
/**
 * 圧縮後の画像情報やにゃ。
 * base64 はそのまま img.src に入れられる Data URL 形式やにゃ。
 */
...
export async function compressImageToJpeg(
     ...
     */
    let { width, height } = calculateContainSize(
        image.naturalWidth,
        image.naturalHeight,
        options.maxWidth,
        options.maxHeight
    ...
    while (
        ...
        width = Math.max(
            options.minWidth,
            Math.round(width * options.downscaleRatio)
        );

        height = Math.max(
            options.minHeight,
            Math.round(height * options.downscaleRatio)
        );

        canvas = drawImageToCanvas(
            image,
            width,
            height,
            options.backgroundColor
...
function validateImageFile(file: File): void {
    ...
    if (!isSupportedByMimeType && !isSupportedByExtension) {
        throw new Error(
            "対応していない画像形式です。png / jpg / jpeg / webp / gif を選択してください。"
        );
    }
}
    ...
    return new Promise((resolve, reject) => {
        canvas.toBlob(
            (blob) => {
                if (!blob) {
                    reject(
                        new Error("Canvas の JPEG 変換に失敗しました。")
                    );
                    return;
                }
 ...
 * @returns 圧縮後 Blob
 */
async function compressCanvasByQuality(
    canvas: HTMLCanvasElement,
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = () => {
            if (typeof reader.result !== "string") {
                reject(
                    new Error("Base64 文字列への変換に失敗しました。")
                );
                return;
            }
