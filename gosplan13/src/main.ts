 ...
 */
const previewFocusExitButton =
    document.querySelector<HTMLButtonElement>(
        "#preview-focus-exit-button"
    );
...
if (
    ...
    throw new Error(
        "レイアウト制御に必要な要素が見つかりません。"
    );
}

if (!copyMenuButton || !copyMenu) {
    throw new Error(
        "複製メニューに必要な要素が見つかりません。"
    );
}
...
async function handleImageChange(
    ...
    const imagePreview =
        productLabel.querySelector<HTMLImageElement>(
